...
# 2. Import data ---------------------------------------------------------
km_data_directory <- here("data", "KM digitized")
ipd_directory <- here("data", "IPD reconstructed")
FP_data_directory <- here("data", "FP data")
outcome_names <- "OS"
study_names <- list()
...
#13 ARASENS ----------------------------------------------
arasens <- read_csv(here("data", "arasens_OS.csv"))
...
# Darolutamide arm
natrisk_dara <- read_excel(here("data", "KM digitized", "ARASENS daralutamide OS.xlsx"), 
                           sheet = 2)
...
write.csv(FP_data_dara[-nrow(FP_data_dara),],
          here("data", "FP data", "OS_ARASENS_daralutamide_FPdata.csv"))

# Placebo arm
natrisk_pcb <- read_excel(here("data", "KM digitized", "ARASENS ADT+Doc OS.xlsx"), 
                           sheet = 2)
...
write.csv(FP_data_pcb, here("data", "FP data", "OS_ARASENS_ADT+Doc_FPdata.csv"))

#11. Merge all datasets-------------------------------------
# ARASENS; arm1: Daro; arm2: ADT+Doc
study1_a1 <- read_csv(here("data", "FP data", 
                           "OS_ARASENS_daralutamide_FPdata.csv"), col_select = -1)
study1_a1

study1_a2 <- read_csv(here("data", "FP data", 
                           "OS_ARASENS_ADT+Doc_FPdata.csv"), col_select = -1)
study1_a2$a <- 2

# ARCHES; arm1: enza+ADT; arm2: ADT
study2_a1 <- read_csv(here("data", "FP data", 
                           "OS_ARCHES_ENZA+ADT_FPdata.csv"), col_select = -1)
study2_a1$s <- 2

study2_a2 <- read_csv(here("data", "FP data", 
...
study3_a2 <- read_csv(here("data", "FP data", 
                           "OS_CHAARTED_ADT_FPdata.csv"), col_select = -1)
study3_a2$s <- 3
study3_a2$a <- 2

# ENZAMET; arm1:enza+ADT; arm2:ADT
study4_a1 <- read_csv(here("data", "FP data", 
...
study5_a1 <- read_csv(here("data", "FP data", 
...
study6_a1 <- read_csv(here("data", "FP data", 
...
write_csv(studies, here("data", "FP data", "FP_data_OS.csv"))
