...
source(here("Fractional polynomials/code", "set_FP_data.R")) 

# 2. Import data ---------------------------------------------------------
km_data_directory <- here("Fractional polynomials/data", "KM digitized")
ipd_directory <- here("Fractional polynomials/data", "IPD reconstructed")
FP_data_directory <- here("Fractional polynomials/data", "FP data")
outcome_names <- "PFS"
study_names <- list()
...
#2.12 ARASENS ----------------------------------------------
arasens <- read_csv(here("Fractional polynomials/data", "arasens_PFS.csv"))
...
natrisk_dara <- read_excel(here("Fractional polynomials/data", "KM digitized", "ARASENS daralutamide PFS.xlsx"), 
                           sheet = 2)
...
write.csv(FP_data_dara[-nrow(FP_data_dara),],
          here("Fractional polynomials/data", "FP data", "PFS_ARASENS_daralutamide_FPdata.csv"))

# Placebo arm
natrisk_pcb <- read_excel(here("Fractional polynomials/data", "KM digitized", "ARASENS ADT+Doc PFS.xlsx"), 
                          sheet = 2)
...
write.csv(FP_data_pcb, here("Fractional polynomials/data", "FP data", "PFS_ARASENS_ADT+Doc_FPdata.csv"))

#3. Merge all datasets-------------------------------------
# ARASENS; arm1: Daro; arm2: ADT+Doc
study1_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ARASENS_daralutamide_FPdata.csv"), col_select = -1)
study1_a1

study1_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ARASENS_ADT+Doc_FPdata.csv"), col_select = -1)
study1_a2$a <- 2

# ARCHES; arm1: Enza; arm2: ADT
study2_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ARCHES_ENZA+ADT_FPdata.csv"), col_select = -1)
study2_a1$s <- 2

study2_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ARCHES_ADT_FPdata.csv"), col_select = -1)
study2_a2$s <- 2
study2_a2$a <- 2

# CHAARTED; arm1: Doc+ADT; arm2: ADT
study3_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_CHAARTED_ADT+Doc_FPdata.csv"), col_select = -1)
study3_a1$s <- 3

study3_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_CHAARTED_ADT_FPdata.csv"), col_select = -1)
study3_a2$s <- 3
study3_a2$a <- 2

# ENZAMET no doc; arm1: enza+ADT, arm2: ADT
study4_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ENZAMET_enzalutamide_FPdata.csv"), col_select = -1)
study4_a1$s <- 4

study4_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ENZAMET_ADT_FPdata.csv"), col_select = -1)
study4_a2$s <- 4
study4_a2$a <- 2

# ENZAMET doc; arm1: doc+ADT vs arm2: Enzalutamide + Doc + ADT
study5_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ENZAMET_ADT+Doc_FPdata.csv"), col_select = -1)
study5_a1$s <- 5

study5_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_ENZAMET_enzalutamide+Doc_FPdata.csv"), col_select = -1)
study5_a2$s <- 5
study5_a2$a <- 2

# GETUG; arm1: ADT+Doc; arm2: ADT
study6_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_GETUG_ADT+Doc_FPdata.csv"), col_select = -1)
study6_a1$s <- 6

study6_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_GETUG_ADT_FPdata.csv"), col_select = -1)
study6_a2$s <- 6
study6_a2$a <- 2

# LATITUDE; arm1: abi; arm2: ADT
study7_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_LATITUDE_AA+P+ADT_FPdata.csv"), col_select = -1)
study7_a1$s <- 7

study7_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_LATITUDE_ADT_FPdata.csv"), col_select = -1)
study7_a2$s <- 7
study7_a2$a <- 2

# PEACE, arm1: Doc+ADT vs arm2: AA+Doc+ADT
study8_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_PEACE_ADT+Doc_FPdata.csv"), col_select = -1)
study8_a1$s <- 8

study8_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_PEACE_AA+ADT+Doc_FPdata.csv"), col_select = -1)
study8_a2$s <- 8
study8_a2$a <- 2

# STAMPEDE-3; arm1: Doc+ADT; arm2: ADT
study9_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                            "PFS_STAMPEDE-3_ADT+Doc_FPdata.csv"), col_select = -1)
study9_a1$s <- 9

study9_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                            "PFS_STAMPEDE-3_ADT_FPdata.csv"), col_select = -1)
study9_a2$s <- 9
study9_a2$a <- 2

# STAMPEDE-4; arm1: abi+ADT; arm2: Doc+ADT
study10_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_STAMPEDE-4_ADT+AAP_FPdata.csv"), col_select = -1)
study10_a1$s <- 10

study10_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                           "PFS_STAMPEDE-4_ADT+DocP_FPdata.csv"), col_select = -1)
study10_a2$s <- 10
study10_a2$a <- 2

# TITAN; arm1: apalutamide; arm2: ADT
study11_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                            "PFS_TITAN_apalutamide_FPdata.csv"), col_select = -1)
study11_a1$s <- 11

study11_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
                            "PFS_TITAN_ADT_FPdata.csv"), col_select = -1)
study11_a2$s <- 11
study11_a2$a <- 2

# Vaishampayan 2021; arm1: enza+ADT; arm2: ADT [double-check this with Howard/Philip]
study12_a1 <- read_csv(here("Fractional polynomials/data", "FP data", 
                            "PFS_Vaishampayan 2021_enzalutamide_FPdata.csv"), col_select = -1)
study12_a1$s <- 12

study12_a2 <- read_csv(here("Fractional polynomials/data", "FP data", 
...
write_csv(studies, here("Fractional polynomials/data", "FP data", "FP_data_PFS.csv"))
