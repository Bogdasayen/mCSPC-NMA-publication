...
model_normal_identity_re <- function()
{
  ...
  d[3] ~ dnorm(0, 0.0001) # DAR+DOC+ADT
...
ns <- nrow(mHSPC_OS_data_merged_except_daro)
t  <- array(c(mHSPC_OS_data_merged_except_daro$t1, mHSPC_OS_data_merged_except_daro$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_data_merged_except_daro$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_data_merged_except_daro$se), dim = c(ns, 2))

study_names <- gsub("#", "", mHSPC_OS_data_merged_except_daro$`#ID`)
...
cross_meandiff_re <- cross_effect(bugs_object = bugs_object_re_mams, t_names = c("adt", "doc+adt", "dar+doc+adt", "enz+adt", "abi+adt", "apa+adt", "enz+doc+adt", "abi+doc+adt"), med = TRUE, exp = TRUE)
