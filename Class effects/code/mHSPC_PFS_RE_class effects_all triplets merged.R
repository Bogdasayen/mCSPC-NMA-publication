...
# Load the data 
mHSPC_PFS_class_effects_all_triplets_merged <- read_excel(here("Class effects/data", "mHSPC PFS_class effects_all triplets merged.xlsx"))
...
ns <- nrow(mHSPC_PFS_class_effects_all_triplets_merged)
t  <- array(c(mHSPC_PFS_class_effects_all_triplets_merged$t1, mHSPC_PFS_class_effects_all_triplets_merged$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_class_effects_all_triplets_merged$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_class_effects_all_triplets_merged$se), dim = c(ns, 2))

study_names <- gsub("#", "", mHSPC_PFS_class_effects_all_triplets_merged$`#ID`)
...
bugs_object_re <- bugs(data = bugs_data, inits = bugs_inits,
                       parameters.to.save = c("d", "totresdev", "doublet_mean", "triplet_mean", "rk", "best", "prob"),
...
cross_meandiff_re_pfs_base <- cross_effect(bugs_object = bugs_object_re_mams, t_names = c("adt", "doc+adt", "dar+doc+adt", "enz+adt", "abi+adt", "apa+adt", "enz+doc+adt", "abi+doc+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_re_pfs_base, file = "cross_meandiff_re_pfs_base.csv")

calculate_sucra(bugs_object = bugs_object_re, bugs_data = bugs_data, t_names = c("adt", "doc+adt", "dar+doc+adt", "enz+adt", "abi+adt", "apa+adt", "enz+doc+adt", "abi+doc+adt"))
calculate_sucra(bugs_object = bugs_object_re_mams, bugs_data = bugs_data_mams, t_names = c("adt", "doc+adt", "dar+doc+adt", "enz+adt", "abi+adt", "apa+adt", "enz+doc+adt", "abi+doc+adt"))
