...
model_normal_identity_fe <- function()
{
  ...
  lhr_adtdocdar <- doublet_mean - d[3]
  hr_triplet <- exp(lhr_triplet)
  hr_adt <- exp(lhr_adt)
  hr_adtdoc <- exp(lhr_adtdoc)
  hr_adtdocdar <- exp(lhr_adtdocdar)
...
bugs_object_fe <- bugs(data = bugs_data, inits = bugs_inits,
                     parameters.to.save = c("d", "doublet_mean", "triplet_mean", "hr_adtdocdar", "hr_triplet", "hr_adt", "hr_adtdoc", "totresdev", "rk", "best", "prob"), #EK added
...
bugs_object_fe_mams <- bugs(data = bugs_data_mams, inits = bugs_inits,
                       parameters.to.save = c("d", "doublet_mean", "triplet_mean", "hr_adtdocdar", "hr_triplet", "hr_adt", "hr_adtdoc", "totresdev", "rk", "best", "prob"),
