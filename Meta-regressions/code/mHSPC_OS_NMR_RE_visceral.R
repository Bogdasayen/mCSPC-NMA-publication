...
# Load the data 
mHSPC_OS_visceral <- read_excel(here("Meta-regressions/data", "mHSPC OS visceral.xlsx"))
...
ns <- nrow(mHSPC_OS_visceral)
t  <- array(c(mHSPC_OS_visceral$t1, mHSPC_OS_visceral$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_visceral$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_visceral$se), dim = c(ns, 2))
x <- mHSPC_OS_visceral$x
...
study_names <- gsub("#", "", mHSPC_OS_visceral$`#ID`)
