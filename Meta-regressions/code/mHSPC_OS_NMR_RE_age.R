...
# Load the data 
mHSPC_OS_age_data <- read_excel(here("Meta-regressions/data", "mHSPC OS age data.xlsx"))
...
ns <- nrow(mHSPC_OS_age_data)
t  <- array(c(mHSPC_OS_age_data$t1, mHSPC_OS_age_data$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_age_data$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_age_data$se), dim = c(ns, 2))
x <- mHSPC_OS_age_data$x
...
study_names <- gsub("#", "", mHSPC_OS_age_data$`#ID`)
...
cross_meandiff_re_nmr_age <- cross_effect(bugs_object = bugs_object_re_mams, t_names = c(1:8), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_re_nmr_age, file = "cross_meandiff_re_nmr_age.csv")
