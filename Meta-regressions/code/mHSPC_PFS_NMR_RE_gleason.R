...
# Load the data 
mHSPC_PFS_gleason <- read_excel(here("Meta-regressions/data", "mHSPC PFS gleason.xlsx"))
...
ns <- nrow(mHSPC_PFS_gleason)
t  <- array(c(mHSPC_PFS_gleason$t1, mHSPC_PFS_gleason$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_gleason$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_gleason$se), dim = c(ns, 2))
x <- mHSPC_PFS_gleason$x
...
study_names <- gsub("#", "", mHSPC_PFS_gleason$`#ID`)
