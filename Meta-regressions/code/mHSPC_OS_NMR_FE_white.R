...
# Load the data 
mHSPC_OS_white <- read_excel(here("Meta-regressions/data", "mHSPC OS white.xlsx"))
...
ns <- nrow(mHSPC_OS_white)
t  <- array(c(mHSPC_OS_white$t1, mHSPC_OS_white$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_white$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_white$se), dim = c(ns, 2))
x <- mHSPC_OS_white$x
...
study_names <- gsub("#", "", mHSPC_OS_white$`#ID`)
