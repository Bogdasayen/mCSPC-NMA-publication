...
# Load the data 
mHSPC_OS_ECOG <- read_excel(here("Meta-regressions/data", "mHSPC OS ECOG.xlsx"))
...
ns <- nrow(mHSPC_OS_ECOG)
t  <- array(c(mHSPC_OS_ECOG$t1, mHSPC_OS_ECOG$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_ECOG$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_ECOG$se), dim = c(ns, 2))
...
study_names <- gsub("#", "", mHSPC_OS_ECOG$`#ID`)
