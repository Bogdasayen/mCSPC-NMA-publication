...
ns <- nrow(mHSPC_OS_volume_data_PO_NMR)
t  <- array(c(mHSPC_OS_volume_data_PO_NMR$t1, mHSPC_OS_volume_data_PO_NMR$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_volume_data_PO_NMR$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_volume_data_PO_NMR$se), dim = c(ns, 2))
x <- mHSPC_OS_volume_data_PO_NMR$x
...
study_names <- gsub("#", "", mHSPC_OS_volume_data_PO_NMR$`#ID`)
...
cross_meandiff_fe_OS_volume <- cross_effect(bugs_object = bugs_object_fe_mams, t_names = c(1:8), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_OS_volume, file = "cross_meandiff_fe_OS_volume.csv")
