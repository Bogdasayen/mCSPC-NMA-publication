...
ns <- nrow(mHSPC_PFS_white_data_PO_NMR)
t  <- array(c(mHSPC_PFS_white_data_PO_NMR$t1, mHSPC_PFS_white_data_PO_NMR$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_white_data_PO_NMR$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_white_data_PO_NMR$se), dim = c(ns, 2))
x <- mHSPC_PFS_white_data_PO_NMR$x
...
study_names <- gsub("#", "", mHSPC_PFS_white_data_PO_NMR$`#ID`)
...
cross_meandiff_re_nmr_pfs_white <- cross_effect(bugs_object = bugs_object_re_mams, t_names = c(1:8), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_re_nmr_pfs_white, file = "cross_meandiff_re_nmr_pfs_white.csv")
