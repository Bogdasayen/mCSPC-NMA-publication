...
# Load the data 
mHSPC_OS_volume <- read_excel(here("Meta-regressions/data", "mHSPC OS volume.xlsx"))
...
ns <- nrow(mHSPC_OS_volume)
t  <- array(c(mHSPC_OS_volume$t1, mHSPC_OS_volume$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_volume$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_volume$se), dim = c(ns, 2))
x <- mHSPC_OS_volume$x
...
study_names <- gsub("#", "", mHSPC_OS_volume$`#ID`)
