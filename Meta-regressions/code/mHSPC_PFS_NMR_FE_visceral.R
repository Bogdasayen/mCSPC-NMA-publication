...
# Load the data 
mHSPC_PFS_visceral <- read_excel(here("Meta-regressions/data", "mHSPC PFS visceral.xlsx"))
...
ns <- nrow(mHSPC_PFS_visceral)
t  <- array(c(mHSPC_PFS_visceral$t1, mHSPC_PFS_visceral$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_visceral$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_visceral$se), dim = c(ns, 2))
x <- mHSPC_PFS_visceral$x
...
study_names <- gsub("#", "", mHSPC_PFS_visceral$`#ID`)
