...
# Load the data 
mHSPC_PFS_docetaxel <- read_excel(here("Meta-regressions/data", "mHSPC PFS docetaxel.xlsx"))
...
ns <- nrow(mHSPC_PFS_docetaxel)
t  <- array(c(mHSPC_PFS_docetaxel$t1, mHSPC_PFS_docetaxel$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_docetaxel$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_docetaxel$se), dim = c(ns, 2))
x <- mHSPC_PFS_docetaxel$x
...
study_names <- gsub("#", "", mHSPC_PFS_docetaxel$`#ID`)
