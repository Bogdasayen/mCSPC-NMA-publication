...
# Load the data 
mHSPC_PFS_m1 <- read_excel(here("Meta-regressions/data", "mHSPC PFS m1.xlsx"))
...
ns <- nrow(mHSPC_PFS_m1)
t  <- array(c(mHSPC_PFS_m1$t1, mHSPC_PFS_m1$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_m1$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_m1$se), dim = c(ns, 2))
x <- mHSPC_PFS_m1$x
...
study_names <- gsub("#", "", mHSPC_PFS_m1$`#ID`)
