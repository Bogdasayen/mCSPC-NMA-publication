...
# Load the data 
mHSPC_OS_docetaxel <- read_excel(here("Meta-regressions/data", "mHSPC OS docetaxel.xlsx"))
...
ns <- nrow(mHSPC_OS_docetaxel)
t  <- array(c(mHSPC_OS_docetaxel$t1, mHSPC_OS_docetaxel$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_docetaxel$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_docetaxel$se), dim = c(ns, 2))
x <- mHSPC_OS_docetaxel$x
...
study_names <- gsub("#", "", mHSPC_OS_docetaxel$`#ID`)
