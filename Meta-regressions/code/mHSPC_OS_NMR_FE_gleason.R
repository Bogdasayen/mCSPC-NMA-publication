...
# Load the data 
mHSPC_OS_gleason <- read_excel(here("Meta-regressions/data", "mHSPC OS gleason.xlsx"))
...
ns <- nrow(mHSPC_OS_gleason)
t  <- array(c(mHSPC_OS_gleason$t1, mHSPC_OS_gleason$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_gleason$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_gleason$se), dim = c(ns, 2))
x <- mHSPC_OS_gleason$x
...
study_names <- gsub("#", "", mHSPC_OS_gleason$`#ID`)
