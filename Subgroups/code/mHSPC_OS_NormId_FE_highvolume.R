...
# Load the data 
mHSPC_OS_data_high_volume <- read_excel(here("Subgroups/data", "mHSPC OS data high_volume.xlsx"))
...
ns <- nrow(mHSPC_OS_data_high_volume)
nt <- max(mHSPC_OS_data_high_volume$t1)
t  <- array(c(mHSPC_OS_data_high_volume$t1, mHSPC_OS_data_high_volume$t2), dim = c(ns, 2)) 
y  <- array(c(rep(0, ns), mHSPC_OS_data_high_volume$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_data_high_volume$se), dim = c(ns, 2))
...
cross_meandiff_fe_highvol <- cross_effect(bugs_object = bugs_object_fe, t_names = c("daro+doce+adt", "doce+adt", "enza+adt", "adt", "apa+adt", "abi+doce+adt", "abi+adt", "enza+doce+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_highvol, file = "cross_meandiff_fe_highvol.csv")
