# P Orishaba - Running a random effects, Normal likelihood, identity link NMA in R2OpenBUGS
...
# Load the data 
mHSPC_OS_data_gleason_greater8 <- read_excel(here("Subgroups/data", "mHSPC OS data gleason_greater8.xlsx"))
...
ns <- nrow(mHSPC_OS_data_gleason_greater8)
t  <- array(c(mHSPC_OS_data_gleason_greater8$t1, mHSPC_OS_data_gleason_greater8$t2), dim = c(ns, 2)) 
nt <- max(t)
y  <- array(c(rep(0, ns), mHSPC_OS_data_gleason_greater8$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_data_gleason_greater8$se), dim = c(ns, 2))
...
cross_meandiff_re_gleason8 <- cross_effect(bugs_object = bugs_object_re, t_names = c("daro+doce+adt", "doce+adt", "enza+adt", "adt", "abi+adt", "apa+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_re_gleason8, file = "cross_meandiff_re_gleason8.csv")
