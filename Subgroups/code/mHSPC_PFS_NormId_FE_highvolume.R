...
nt <- max(mHSPC_PFS_data_high_volume$t1)
...
cross_meandiff_fe_pfs_highvol <- cross_effect(bugs_object = bugs_object_fe, t_names = c("daro+doce+adt", "doce+adt", "enza+adt", "adt", "apa+adt", "abi+doce+adt", "abi+adt", "enza+doce+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_pfs_highvol, file = "cross_meandiff_fe_pfs_highvol.csv")
