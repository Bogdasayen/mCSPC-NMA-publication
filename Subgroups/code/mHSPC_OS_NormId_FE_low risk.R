...
# Load the data 
mHSPC_OS_data_low_risk <- read_excel(here("Subgroups/data", "mHSPC OS data low risk.xlsx"))
...
ns <- nrow(mHSPC_OS_data_low_risk)
nt <- max(mHSPC_OS_data_low_risk$t1)
t  <- array(c(mHSPC_OS_data_low_risk$t1, mHSPC_OS_data_low_risk$t2), dim = c(ns, 2)) 
y  <- array(c(rep(0, ns), mHSPC_OS_data_low_risk$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_data_low_risk$se), dim = c(ns, 2))
...
cross_meandiff_fe_OS_lowrisk <- cross_effect(bugs_object = bugs_object_fe, t_names = c("daro+doce+adt", "doce+adt", "adt", "apa+adt", "abi+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_OS_lowrisk, file = "cross_meandiff_fe_OS_lowrisk.csv")

calculate_sucra(bugs_object = bugs_object_fe, bugs_data = bugs_data, t_names = c("daro+doce+adt", "doce+adt", "adt", "apa+adt", "abi+adt"))
