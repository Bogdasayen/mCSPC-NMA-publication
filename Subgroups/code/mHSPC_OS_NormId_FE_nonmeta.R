...
# Load the data 
mHSPC_OS_data_non_meta <- read_excel(here("Subgroups/data", "mHSPC OS data non_meta.xlsx"))
...
ns <- nrow(mHSPC_OS_data_non_meta)
nt <- max(mHSPC_OS_data_non_meta$t1)
t  <- array(c(mHSPC_OS_data_non_meta$t1, mHSPC_OS_data_non_meta$t2), dim = c(ns, 2)) 
y  <- array(c(rep(0, ns), mHSPC_OS_data_non_meta$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_data_non_meta$se), dim = c(ns, 2))
...
cross_meandiff_fe_OS_nonmeta <- cross_effect(bugs_object = bugs_object_fe, t_names = c("daro+doce+adt", "doce+adt", "adt", "abi+adt", "apa+adt", "enza+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_OS_nonmeta, file = "cross_meandiff_fe_OS_nonmeta.csv")

calculate_sucra(bugs_object = bugs_object_fe, bugs_data = bugs_data, t_names = c("daro+doce+adt", "doce+adt", "adt", "abi+adt", "apa+adt", "enza+adt"))
