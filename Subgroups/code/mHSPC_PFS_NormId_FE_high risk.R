...
# Load the data 
mHSPC_PFS_data_high_risk <- read_excel(here("Subgroups/data", "mHSPC PFS data high risk.xlsx"))
...
t  <- array(c(mHSPC_PFS_data_high_risk$t1, mHSPC_PFS_data_high_risk$t2), dim = c(ns, 2)) 
y  <- array(c(rep(0, ns), mHSPC_PFS_data_high_risk$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_data_high_risk$se), dim = c(ns, 2))
...
cross_meandiff_fe_pfs_highrisk <- cross_effect(bugs_object = bugs_object_fe, t_names = c("daro+doce+adt", "doce+adt", "enza+adt", "adt", "abi+adt"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_pfs_highrisk, file = "cross_meandiff_fe_pfs_highrisk.csv")
