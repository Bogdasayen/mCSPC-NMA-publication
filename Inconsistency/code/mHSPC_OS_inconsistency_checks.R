...
temp <- mHSPC_OS_data_PO$t1
mHSPC_OS_data_PO <- transform(mHSPC_OS_data_PO, t1 = ifelse(t1 > t2, t2, t1))
mHSPC_OS_data_PO <- transform(mHSPC_OS_data_PO, t2 = ifelse(t2 == t1, temp, t2))
...
study_names <- gsub("#", "", mHSPC_OS_data_PO$X.ID)
...
direct_evidence <- as.data.frame(mHSPC_OS_data_PO[(mHSPC_OS_data_PO$t1 == 2 & mHSPC_OS_data_PO$t2 == 4),])
direct_evidence$t1 <- 1
direct_evidence$t2 <- 2
#Treatments 2 and 4 rerabelled 1 and 2
...
indirect_evidence <- as.data.frame(mHSPC_OS_data_PO[(mHSPC_OS_data_PO$t2 == 5),])
