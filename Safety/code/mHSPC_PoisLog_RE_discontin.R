...
model_poisson_log_re <- function(){ # *** PROGRAM STARTS
  for(i in 1:ns){ # LOOP THROUGH STUDIES
    ...
    mu[i] ~ dnorm(0,.05) # vague priors for all trial baselines
  ...
  for (k in 2:nt){ d[k] ~ dnorm(0,.05) } # vague priors for treatment effects
  ...
  var_d <- pow(1.52, -2) # code for 1/(1.41*1.41)
  var ~ dlnorm(-1.87, var_d) #lognormal distribution
  tau <- pow(var, -1)
...
ns <- nrow(discontinuation_due_to_AE)
nt <- max(discontinuation_due_to_AE$t1)
t  <- array(c(discontinuation_due_to_AE$t1, discontinuation_due_to_AE$t2), dim = c(ns, 2)) 
r  <- array(c(discontinuation_due_to_AE$r1, discontinuation_due_to_AE$r2), dim = c(ns, 2)) 
n  <- array(c(discontinuation_due_to_AE$n1, discontinuation_due_to_AE$n2), dim = c(ns, 2))
E <- array(c(discontinuation_due_to_AE$E1, discontinuation_due_to_AE$E2), dim = c(ns, 2))
...
raw_rr <- cbind(t, ((r/E)[,2])/((r/E)[,1]), discontinuation_due_to_AE$`#ID`)
...
inits1 <- list(d=c( NA, rep(0, nt - 1)), var=1)
inits2 <- list(d=c( NA, rep(-1, nt - 1)), var=2)
inits3 <- list(d=c( NA, rep(2, nt - 1)), var=0.5)
...
cross_hr_re_discontin <- cross_effect(bugs_object = bugs_object_re, t_names = c("adt", "doce+adt", "enza+adt", "daro+doce+adt", "abi+adt", "apa+adt", "abi+doce+adt"), med = TRUE, exp = FALSE)
write.csv(x = cross_hr_re_discontin, file = "cross_hr_re_discontin.csv")

calculate_sucra(bugs_object = bugs_object_re, bugs_data = bugs_data, t_names = c("adt", "doce+adt", "enza+adt", "daro+doce+adt", "abi+adt", "apa+adt", "abi+doce+adt"))
